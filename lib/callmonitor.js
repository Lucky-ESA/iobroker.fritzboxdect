...
class Callmonitor extends EventEmitter {
    ...
    setStatus(val) {
        this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.status`, { val: val, ack: true });
    }
    ...
    processLine(line) {
        ...
        if (data && data.error != null) {
            this.adapter.log.warn(`ERROR_CALL: ${JSON.stringify(data)}`);
        }
        ...
        if (data != null && typeof data === "number" && this.call_events[data]) {
            if (this.call_events[data].kind === "HangUp") {
                delete this.call_events[data];
            }
        }
    ...
    parseLine(line) {
        ...
        this.setAction(sp);
        if (!this.call_events[sp[2]] || this.call_events[sp[2]] == null) {
    ...
    setAction(sp) {
        if (sp[1] && typeof sp[1] === "string") {
            this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.action`, {
                val: sp[1].toUpperCase(),
                ack: true,
            });
            this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.action_id`, {
                val: parseInt(sp[2]),
                ack: true,
            });
        }
    ...
    async setStates(folder, val, connectionId) {
        if (val.called != null) {
            ...
            if (val.calledname == "") {
                val.calledname = "Unknown";
            }
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.called`, {
                val: val.called,
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.calledname`, {
                val: val.calledname,
                ack: true,
            });
        }
        if (val.caller != null) {
            ...
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.caller`, {
                val: val.caller,
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.callername`, {
                val: val.callername,
                ack: true,
            });
        }
        if (val.extension != null) {
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.extension`, {
                val: val.extension,
                ack: true,
            });
        }
        if (val.duration != null && folder == "lastcall") {
            await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.duration`, {
                val: val.duration,
                ack: true,
            });
        }
        await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.id`, { val: val.id, ack: true });
        await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.timestamp`, {
            val: val.timestamp,
            ack: true,
        });
        await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.json`, {
            val: JSON.stringify(val),
            ack: true,
        });
        await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.type`, {
            val: val.kind,
            ack: true,
        });
        await this.adapter.setState(`${this.config.dp}.TR_064.Callmonitor.${folder}.sip`, { val: val.sip, ack: true });
        if (folder == "lastcall") {
            this.call_events[connectionId] = null;
        }
    }
