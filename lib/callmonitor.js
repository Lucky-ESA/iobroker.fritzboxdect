...
class Callmonitor extends EventEmitter {
    constructor(config, adapter) {
        ...
        this.callList = new calllist(config, adapter);
        this.phonelist = null;
        this.call_events = {};
    }
    ...
    createEvent(evt, date, connectionId, line, splitLines) {
        ...
        switch (evt) {
            ...
            case "Call":
                this.call_events[connectionId].kind = evt;
                this.call_events[connectionId].extension = splitLines[3];
                this.call_events[connectionId].caller = splitLines[4];
                this.call_events[connectionId].callee = splitLines[5];
                this.call_events[connectionId].sip = splitLines[6];
                this.setStates("outbound", this.call_events[connectionId], connectionId);
                return this.call_events[connectionId];
            case "PickUp":
                this.call_events[connectionId].kind = evt;
                this.call_events[connectionId].callee = splitLines[4];
                this.call_events[connectionId].extension = splitLines[3];
                this.setStates("connect", this.call_events[connectionId], connectionId);
                return this.call_events[connectionId];
            case "Ring":
                this.call_events[connectionId].kind = evt;
                this.call_events[connectionId].caller = splitLines[3];
                this.call_events[connectionId].caller = splitLines[4];
    ...
    async setStates(folder, val, connectionId) {
        if (val.callee != null) {
            if (this.phonelist != null && val.calleename == "") {
                if (this.phonelist[val.callee]) {
                    val.calleename = this.phonelist[val.callee];
                } else if (this.phonelist[this.callList.getCountryCode(val.callee)]) {
                    val.calleename = this.phonelist[this.callList.getCountryCode(val.callee)];
                } else {
                    for (const phonenr in this.phonelist) {
                        if (this.phoneNumbersMatch(val.callee, phonenr)) {
                            val.calleename = this.phonelist[phonenr];
                            break;
                        }
                    }
                }
            }
            if (val.calleename == "") val.calleename = "Unknown";
            await this.adapter.setStateAsync(`${this.config.dp}.TR_064.Callmonitor.${folder}.callee`, val.callee, true);
            await this.adapter.setStateAsync(
                `${this.config.dp}.TR_064.Callmonitor.${folder}.calleename`,
                val.calleename,
                true,
            );
        }
