...
class Callmonitor extends EventEmitter {
    ...
    processLine(line) {
        ...
        this.adapter.log.debug(`LINS: ${line}`);
    ...
    createEvent(evt, date, connectionId, line, splitLines) {
        this.call_events[connectionId].rawdata = line;
        ...
        switch (evt) {
            ...
            case "PickUp":
                ...
                return this.call_events[connectionId];
            case "Ring":
                if (!splitLines[3].startsWith("00") && splitLines[3].startsWith("0")) {
                    if (this.constants[`Z${splitLines[3].substring(0, 6)}`]) {
                        this.call_events[connectionId].prefix = splitLines[3].substring(0, 6);
                        this.call_events[connectionId].state = this.constants[`Z${splitLines[3].substring(0, 6)}`];
                    } else if (this.constants[`Z${splitLines[3].substring(0, 5)}`]) {
                        this.call_events[connectionId].prefix = splitLines[3].substring(0, 5);
                        this.call_events[connectionId].state = this.constants[`Z${splitLines[3].substring(0, 5)}`];
                    } else if (this.constants[`Z${splitLines[3].substring(0, 4)}`]) {
                        this.call_events[connectionId].prefix = splitLines[3].substring(0, 4);
                        this.call_events[connectionId].state = this.constants[`Z${splitLines[3].substring(0, 4)}`];
                    } else if (this.constants[`Z${splitLines[3].substring(0, 3)}`]) {
                        this.call_events[connectionId].prefix = splitLines[3].substring(0, 3);
                        this.call_events[connectionId].state = this.constants[`Z${splitLines[3].substring(0, 3)}`];
                    }
                }
                this.call_events[connectionId].duration = 0;
                this.call_events[connectionId].extension = 0;
                this.call_events[connectionId].kind = evt;
                this.call_events[connectionId].called = splitLines[3].replace(/#/g, "");
                this.call_events[connectionId].caller = splitLines[4].replace(/#/g, "");
