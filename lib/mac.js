...
class MACMonitor extends EventEmitter {
    ...
    async chechStatus() {
        ...
        if (!this.checkvalue[object_path] || this.checkvalue[object_path] != count) {
            this.adapter.setState(object_path, count, true);
        }
        this.adapter.setState(`${this.config.dp}.TR_064.Presence.status_all_json`, JSON.stringify(this.all_mac), true);
    }
