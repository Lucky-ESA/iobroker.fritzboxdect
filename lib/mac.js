...
class MACMonitor extends EventEmitter {
    ...
    async setStates(val) {
        ...
        if (!this.checkvalue[object_path] || this.checkvalue[object_path] != JSON.stringify(val.ip_fritz)) {
            this.checkvalue[object_path] == JSON.stringify(val.ip_fritz);
            await this.adapter.setStateAsync(object_path, val.ip_fritz, true);
        }
