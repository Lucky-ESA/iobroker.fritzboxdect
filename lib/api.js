    ...
    async updateDECT() {
        ...
        if (devices && devices.devicelist && devices.devicelist.device) {
            ...
            if (this.dect != dect) {
                ...
                await this.adapter.setStateAsync(`${this.config.dp}.DECT_Control.interval`, "Update DECT DONE", true);
            } else {
                await this.adapter.setStateAsync(`${this.config.dp}.DECT_Control.interval`, "No DECT update", true);
            ...
            if (device.functionbitmask != 1) {
                ...
                for (const key in device) {
                    ...
                    if (typeof device[key] === "object") {
                        if (!Array.isArray(device[key])) {
                            for (const subkey in device[key]) {
                                if (typeof device[key][subkey] === "object") {
                                    ...
                                    if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                                        ...
                                        await this.adapter.setStateAsync(dp, device[key][subkey], true);
                                    }
                                }
                            }
                        } else {
                            for (const button of device[key]) {
                                ...
                                for (const keys in button) {
                                    ...
                                    if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                                        this.lastValue[dp] = val;
                                        await this.adapter.setStateAsync(dp, button[keys], true);
                                    }
                                }
                            }
                        }
                        ...
                        if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                            this.lastValue[dp] = val;
                            await this.adapter.setStateAsync(dp, device[key], true);
                        }
                        if (key === "present") {
