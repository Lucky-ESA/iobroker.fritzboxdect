...
class Fritzbox extends EventEmitter {
            ...
            if (typeof device_array[device] === "object") {
                ...
                for (let keys in device_array[device]) {
                    if (keys === "_@attribute") {
                        ...
                        this.adapter.log.info(`VALUE: ${arr}`);
                        keys = "value";
                        ...
                        if (arr && arr.length > 0) {
                            ...
                            this.adapter.log.info(JSON.stringify(static_val));
    ...
    async fritzRequest(methode, url, request) {
        ...
        if (request === "template" && typeof resp === "string") {
            resp = resp.replace(/applymask=/g, "mask=").replace(/<sub_template \/>/g, "<sub_template />");
        }
        try {
    ...
    async own_request(methode, url, data, value) {
        return await this.sendRequest(methode, `${url}${this.config.response_sid.SessionInfo.SID}`, data, value);
    }
