...
class Fritzbox extends EventEmitter {
    ...
    async osversion() {
        const version = await this.sendRequest("GET", "/jason_boxinfo.xml", null, true);
        try {
            const fullVersion = version.match("<j:Version>(.*?)</j:Version>")[1];
    ...
    async login() {
        const login = await this.sendRequest("GET", "/login_sid.lua?version=2", null, true);
        try {
            ...
            const sid = await this.sendRequest("post", "/login_sid.lua?version=2", params, true);
    ...
    async relogin() {
        ...
        if ((!login || login == "BLOCK") && this.restart) {
            this.relogInterval();
        } else {
    ...
    async getStatusULE() {
        ...
        if (resp && resp.statusText === "OK") {
            ...
            this.adapter.log.info("HHH: " + JSON.stringify(response));
    ...
    async createTemperature(color, path, com) {
        ...
        for (const temp of color) {
            if (temp && temp.value != null) {
                this.adapter.log.info(JSON.stringify(com.commons["temp"]));
                if (com && com.commons && com.commons["temp"]) {
                    common = com.commons["temp"];
                    this.adapter.log.info("HH: " + JSON.stringify(common));
            ...
            if (typeof device_array[device] === "object") {
                ...
                for (let keys in device_array[device]) {
                    if (keys === "_@attribute") {
                        ...
                        device_array[device][keys] = device_array[device]["_@attribute"].split(",").map(Number);
                    } else {
                ...
                if (device === "_@attribute") {
                    ...
                    device_array[device] = device_array["_@attribute"].split(",").map(Number);
                } else {
    ...
    async getCommand(methode, url, id, value) {
        //const strcheck = { username: this.clients[fritz].user, sid: this.xmlvalue.sid };
        const resp = await this.sendRequest(
            methode,
            `/webservices/homeautoswitch.lua?${url}${this.config.response_sid.SessionInfo.SID}`,
            null,
        ...
        if (resp && resp.statusText === "OK") {
            this.adapter.log.info(JSON.stringify(resp.data));
        } else {
            this.adapter.log.info(`Cannot sent request! - ${resp}`);
    ...
    async fritzRequest(methode, url, request) {
        ...
        let resp = await this.sendRequest(methode, `${url}${this.config.response_sid.SessionInfo.SID}`, null, true);
        ...
        if (request === "template" && typeof resp === "string") {
            resp = resp.replace(/applymask=/g, "mask=");
        }
        try {
    ...
    async sendRequest(methode, url, data, value) {
        if (data == null) {
            data = "";
        }
    ...
    async sidCheck() {
        this.checkInterval = this.adapter.setInterval(
            async () => {
                ...
                const resp = await this.sendRequest("post", "/login_sid.lua?version=2", params, true);
                const sid = resp.match("<SID>(.*?)</SID>")[1];
    ...
    async start(dev) {
        ...
        this.infoInterval = this.adapter.setInterval(async () => {
            ...
            if (this.config.response_sid.SessionInfo.SID === "0000000000000000") {
                this.adapter.log.info(`Wrong SID!!!`);
                this.check = true;
                this.relogin();
            }
