...
class Fritzbox extends EventEmitter {
    ...
    async sendHTTP(fritz, state, obj) {
        ...
        this.adapter.setState(
            `${this.config.dp}.TR_064.States.sendHTTPResponse`,
        ...
        if (monitor != null && typeof monitor === "string" && monitor.indexOf("LastMonth") != -1) {
            ...
            if (fullConst != null && fullConst[1] != null) {
                try {
                    ...
                    this.adapter.setState(
                        `${this.config.dp}.TR_064.States.Traffic.fritz_monitor`,
                        JSON.stringify(all),
                        true,
                    );
                    all_array.sort((a, b) => a.id - b.id);
                    this.adapter.setState(
                        `${this.config.dp}.TR_064.States.Traffic.fritz_monitor_array`,
                        JSON.stringify(all_array),
                        true,
                    );
                } catch (e) {
    ...
    async updateDECT() {
        if (this.isWork || this.check) {
            this.adapter.setState(
                `${this.config.dp}.DECT_Control.interval`,
                `WORK - ${this.isWork} - ${this.check}`,
                true,
            );
        ...
        if (devices && devices.devicelist && devices.devicelist.device) {
            ...
            if (this.dect != dect) {
                ...
                this.adapter.setState(`${this.config.dp}.DECT_Control.interval`, "Update DECT DONE", true);
            } else {
                this.adapter.setState(`${this.config.dp}.DECT_Control.interval`, "No DECT update", true);
            ...
            if (device.functionbitmask != 1) {
                ...
                for (const key in device) {
                    ...
                    if (typeof device[key] === "object") {
                        if (!Array.isArray(device[key])) {
                                ...
                                if (typeof device[key][subkey] === "object") {
                                    if (subkey === "device" && Array.isArray(device[key][subkey])) {
                                        ...
                                        for (const subsubkey in device[key][subkey]) {
                                            ...
                                            if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                                                this.lastValue[dp] = val;
                                                this.adapter.setState(dp, device[key][subkey][subsubkey], true);
                                            }
                                        }
                                    }
                                } else {
                                    ...
                                    if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                                        ...
                                        this.adapter.setState(dp, device[key][subkey], true);
                                    }
                                }
                            }
                        } else {
                            for (const button of device[key]) {
                                ...
                                for (const keys in button) {
                                    ...
                                    if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                                        this.lastValue[dp] = val;
                                        this.adapter.setState(dp, button[keys], true);
                                    }
                                }
                            }
                        }
                        ...
                        if (!this.lastValue[dp] || this.lastValue[dp] != val) {
                            this.lastValue[dp] = val;
                            this.adapter.setState(dp, device[key], true);
                        }
                        if (key === "present") {
