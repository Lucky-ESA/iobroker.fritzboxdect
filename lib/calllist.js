...
class Calllist extends EventEmitter {
    ...
    async getCallList() {
        ...
        if (callLists != null && typeof callLists === "string" && callLists.indexOf("NewCallListURL") != -1) {
            ...
            if (path && path[1]) {
                this.adapter.log.debug(path);
                let callList = await this.requests(path, null, null, null);
    ...
    async createCallList(result) {
        ...
        if (result && result.root && result.root.call && Array.isArray(result.root.call)) {
            for (const call of result.root.call) {
                call.device = Buffer.from(call.device, "ascii").toString();
                call.name = Buffer.from(call.name, "ascii").toString();
                if (call.type == "1") {
                    ++count_inbound;
                } else if (call.type == "2") {
                    ++count_missed;
    ...
    async getCountry() {
        ...
        if (areaCode != null && typeof areaCode === "string" && areaCode.indexOf("NewX_AVM-DE_OKZ") != -1) {
            // @ts-ignore
            const ac = areaCode.match("<NewX_AVM-DE_OKZ>(.*?)</NewX_AVM-DE_OKZ>")[1];
            this.country["area"] = ac;
            if (areaCode.indexOf("NewX_AVM-DE_OKZPrefix") != -1) {
                // @ts-ignore
                const pac = areaCode.match("<NewX_AVM-DE_OKZPrefix>(.*?)</NewX_AVM-DE_OKZPrefix>")[1];
                this.country["area"] = pac + this.country["area"];
            }
        }
        return true;
    }
    ...
    async getPhoneBook() {
        ...
        if (phonebookNr != null && typeof phonebookNr === "string" && phonebookNr.indexOf("NewPhonebookList") != -1) {
            // @ts-ignore
            const pb = phonebookNr.match("<NewPhonebookList>(.*?)</NewPhonebookList>")[1].split(",");
            if (pb && pb.length > 0) {
                for (const pb_id of pb) {
                    ...
                    if (
                        ...
                        // @ts-ignore
                        const path = phonebookList
                            .match("<NewPhonebookURL>(.*?)</NewPhonebookURL>")[1]
                            .replace(/&amp;/gi, "&");
                        // @ts-ignore
                        this.phonebook[pb_id] = phonebookList.match("<NewPhonebookName>(.*?)</NewPhonebookName>")[1];
                        this.adapter.log.debug(path);
                        let phoneList = await this.requests(path, null, null, null);
