...
class Calllist extends EventEmitter {
    ...
    async createCallList(result) {
        ...
        if (result && result.root && result.root.call && Array.isArray(result.root.call)) {
            this.lastcall.timestamp = result.root.timestamp != null ? parseInt(result.root.timestamp) : 0;
            this.lastcall.id = result.root.call[0] != null ? parseInt(result.root.call[0].id) : 0;
            this.adapter.log.debug(JSON.stringify(`TIMESTAMP: ${JSON.stringify(this.lastcall)}`));
            for (const call of result.root.call) {
                if (this.onunload) break;
            ...
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.incomming.count`, {
                val: count_inbound,
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.outgoing.count`, {
                val: count_outbound,
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.missed.count`, {
                val: count_missed,
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.incomming.json`, {
                val: JSON.stringify(arr_inbound),
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.outgoing.json`, {
                val: JSON.stringify(arr_outbound),
                ack: true,
            });
            await this.adapter.setState(`${this.config.dp}.TR_064.Calllists.missed.json`, {
                val: JSON.stringify(arr_missed),
                ack: true,
            });
        }
    }
    ...
    setPrefix(call) {
        if (call.caller != "" && !call.caller.startsWith("00") && call.caller.startsWith("0")) {
            if (this.constants[`Z${call.caller.substring(0, 6)}`]) {
                call.prefix = call.caller.substring(0, 6);
                call.state = this.constants[`Z${call.caller.substring(0, 6)}`];
            } else if (this.constants[`Z${call.caller.substring(0, 5)}`]) {
                call.prefix = call.caller.substring(0, 5);
                call.state = this.constants[`Z${call.caller.substring(0, 5)}`];
            } else if (this.constants[`Z${call.caller.substring(0, 4)}`]) {
                call.prefix = call.caller.substring(0, 4);
                call.state = this.constants[`Z${call.caller.substring(0, 4)}`];
            } else if (this.constants[`Z${call.caller.substring(0, 3)}`]) {
                call.prefix = call.caller.substring(0, 3);
                call.state = this.constants[`Z${call.caller.substring(0, 3)}`];
            }
        }
        return call;
    }
            ...
            if (Object.keys(phoneNumber).length > 0) {
                for (const attr in phoneNumber) {
                    ...
                    if (this.phonebook[attr]) {
                        bookname = " (" + Buffer.from(this.phonebook[attr], "utf-8").toString() + ")";
                    }
                    if (
                        phoneNumber[attr] &&
                            ...
                            if (contact) {
                                const name =
                                    contact.person && contact.person.realname
                                        ? entities.decodeHTML(contact.person.realname)
                                        : "";
                                const contact_json = {
                    ...
                    if (phonebook_state.length > 0 && this.config.phone) {
                        ...
                        const phoneName =
                            this.phonebook[attr] != null ? Buffer.from(this.phonebook[attr], "utf-8").toString() : "";
