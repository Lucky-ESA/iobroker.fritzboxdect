...
function xml2jsonRecurse(xmlStr, states) {
    ...
    try {
        while (xmlStr.match(/<[^/][^>]*>/)) {
            ...
            if (inner_substring == "FRITZ!DECT 350" && mask == "1") dect350 = inner_substring;
            if (dect350 != "" && tagName == "name" && dect350_name == "") dect350_name = inner_substring.toString();
            if (dect350 != "" && tagName == "battery") dect350_bat = parseInt(inner_substring);
            if (dect350 != "" && tagName == "batterylow") dect350_batlow = inner_substring;
